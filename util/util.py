...
from models.Wavaletplus import WaveletNetPlus
import torch.nn.functional as F
from dataloader.New_PolypDataset import New_PolypDataset
...
def build_data(cfg):
    ...
    elif cfg["DATASET"]["dataset"] == "New_PolypDataset":
        train_set = New_PolypDataset(
            cfg,
            root=cfg["DATASET"]["dataroot"],
            split="train",
        )
        val_set = New_PolypDataset(
            cfg,
            root=cfg["DATASET"]["dataroot"],
            split="test",
        )
    elif cfg["DATASET"]["dataset"] == "New_PolypVideoDataset":
        train_set = New_PolypVideoDataset(
            cfg, root=cfg["DATASET"]["dataroot"], split="train", clip_len=8
        )
        val_set = New_PolypVideoDataset(
            cfg, root=cfg["DATASET"]["dataroot"], split="test", clip_len=8
        )
...
def build_model(train_opt, cfg):
    ...
    if cfg["DATASET"]["dataset"] == "New_PolypVideoDataset":
        model = seg_network(
            WaveletNetPlus(in_channels=1, num_classes=cfg["DATASET"]["num_labels"]),
            train_opt,
            cfg,
        )
    else:
