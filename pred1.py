...
for item in predictions:
    ...
    if args.overlay:
        ...
        low_freq = item["low_freq"].squeeze().cpu().numpy()
        low_freq = (
            low_freq * cfg["DATASET"]["Low_std"] + cfg["DATASET"]["Low_mean"]
        ) * 255

        low_freq = cv2.resize(
            low_freq, (cfg["TRAIN"]["size"][1], cfg["TRAIN"]["size"][0])
        )

        low_freq_dir = os.path.join(save_dir, "low_freq")
        os.makedirs(low_freq_dir, exist_ok=True)
        low_freq_filename = os.path.join(low_freq_dir, f"{base_name}_pred.png")
        cv2.imwrite(low_freq_filename, low_freq)

        high_freq = item["high_freq"].squeeze().cpu().numpy()
        high_freq = (
            high_freq * cfg["DATASET"]["High_std"] + cfg["DATASET"]["High_mean"]
        ) * 255

        high_freq = cv2.resize(
            high_freq, (cfg["TRAIN"]["size"][1], cfg["TRAIN"]["size"][0])
        )

        high_freq_dir = os.path.join(save_dir, "high_freq")
        os.makedirs(high_freq_dir, exist_ok=True)
        high_freq_filename = os.path.join(high_freq_dir, f"{base_name}_pred.png")
        cv2.imwrite(high_freq_filename, high_freq)
