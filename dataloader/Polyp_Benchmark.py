...
class Polyp_Benchmark_Dataset(data.Dataset):

    CLASSES = ["background", "Polyp"]

    def __init__(self, cfg, root="", split="TrainDataset"):
        ...
        if split == "TrainDataset":
            self.image_base = os.path.join(self.root, self.split, "image")
            self.label_base = os.path.join(self.root, self.split, "mask")
        elif split == "TestDataset":
            self.image_base = os.path.join(self.root, self.split, "*", "images")
            self.label_base = os.path.join(self.root, self.split, "*", "masks")
        else:
            raise Exception(
                f"Invalid split name: '{split}'. Expected 'TrainDataset' or 'TestDataset'. "
                "Please check your input and ensure you are using one of the supported split names."
            )
    ...
    def __getitem__(self, index):
        ...
        if self.split == "TrainDataset":
            sample = self.transform_tr(sample)
        elif self.split == "TestDataset":
